...
int **CreateTwoDimensionalArray(unsigned int uiYsize, unsigned int uiXsize){
    ...
    for(uiYindex = 0; uiYindex < uiYsize;uiYindex++) *(ppiHead + uiYindex) = (int *) malloc(uiXsize*sizeof(int));
    return ppiHead;
}
...
double **d_CreateTwoDimensionalArray(unsigned int uiYsize,unsigned int uiXsize){
    ...
    for(uiYindex = 0; uiYindex < uiYsize;uiYindex++) *(ppiHead + uiYindex) = (double *) malloc(uiXsize*sizeof(double));
    return ppiHead;
}
...
char **ch_CreateTwoDimensionalArray(unsigned int uiYsize,unsigned int uiXsize){
    ...
    for(uiYindex = 0; uiYindex < uiYsize;uiYindex++) *(ppiHead + uiYindex) = (char *) malloc(uiXsize*sizeof(char));
    return ppiHead;
}

void DeleteTwoDimensionalArray(double **ppiHead, int uiYsize, int uiXsize){
    int uiYindex;
    for(uiYindex = 0;uiYindex < uiYsize;uiYindex++) free(*(ppiHead + uiYindex));
    free(ppiHead);
}
...
void INV_matrix(double **a,double **inv_a,int size){
    ...
    /* printf("INV-----------------\n");
    for(i=0;i<size;i++){
        for(j=0;j<size;j++) printf(" %f",inv_a[i][j]);
        printf("\n");
    }*/
    //return inv_a;
}
...
void Cholesky(double **A,double **TL,int size){
    ...
    clear_vector(w,size);
    for(i=0;i<size;i++){
        for(m=i;m<size;m++) w[m]=A[i][m];
        for(k=0;k<i;k++){
            temp=L[k][i];
            if(temp!=0){
                for(m=i;m<size;m++) w[m] -= temp*L[k][m];
            }
        }
        w[i]=sqrt(w[i]);
        for(m=i+1;m<size;m++) w[m] /=w[i];
        for(m=i;m<size;m++) L[i][m]=w[m];
        clear_vector(w,size);
    }
